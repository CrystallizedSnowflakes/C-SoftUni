...
namespace _13.HandsOfCards
{
    class HandsOfCards
    {
        static void Main(string[] args)
        {
            ...
            {
                var currentDraw = command[1].Split(new char[] { ' ', ',' }).Where(x => x != "").Distinct().ToList();

                if (players.ContainsKey(command[0]))
                {
                    players[command[0]] = players[command[0]].Concat(currentDraw).Distinct().ToList();
                }
                else
                {
